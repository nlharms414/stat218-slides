...
## Section 2.2 Learning Objectives {.center}
...
-   Know the difference between a Type I and Type II error and what the implications/consequences are.
...
-   Generally refers to mean, median, or mode
...
## Null Distribution (Categorical Variable) {.center}

![](images/2_applet.png){fig-alt="Dot plot where each dot represents one set of 16 attempts by Buzz. Dots form a mound shape between 3 and 11 that peaks between 7 and 8." fig-align="center"}

Here, each dot represents a **simulated statistic.**

## Data Distribution (Numerical Variable) {.center}

![](images/2_gpa.png){fig-alt="Dot plot distribution of Cumulative GPA data from a sample of size 30." fig-align="center"}

Here, each dot represents a data point we collected.

. . .

**Data distributions** only "make sense" when we are working with a **numerical variable.**

## What's the difference? {.center}

<br> We can also display distributions using a **histogram.**
...
::: {.column width="50%"}
-   Each data point represents one simulation (or real life sample!) of a study
